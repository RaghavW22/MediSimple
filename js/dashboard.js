...
(function () {
  ...
  function drawHistoryChart(historyList) {
    ...
    const createChart = (title, data, colorBg, colorBorder) => {
        ...
        wrap.style.background = 'var(--white)';
        ...
        const instance = new Chart(canvas, {
          ...
          options: {
            ...
            maintainAspectRatio: false,
            scales: {
              y: { beginAtZero: title === 'Health Score Trend' }
            },
            plugins: { legend: { display: false } }
          }
        });
  ...
  function renderHistoryLog(historyList) {
    ...
    // Create copy to reverse so most recent is first
    ...
    sorted.forEach((h, i) => {
       ...
       div.innerHTML = `
         ...
         <button class="btn-secondary" style="padding:4px 8px;font-size:12px;" onclick="loadPastReport(${i})">View Details</button>
       `;
  ...
  window.loadPastReport = function(idx) {
      if (window.__msPastHistory && window.__msPastHistory[idx]) {
          ...
          renderAll(pastData);
          showModeBadge('demo', (past.metrics || []).filter(m => m.value !== 'N/A').length);
          document.getElementById('headerSubText').textContent = `Viewing historic lab results from ${past.reportDate}`;
