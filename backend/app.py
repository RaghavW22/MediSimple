...
# ── Database Setup ────────────────────────────────────────────────────────────
DB_FILE = os.path.join(DATABASE_DIR, 'medisimple.db')
def init_db():
...
def api_signup():
    ...
    face_descriptor = data.get('face_descriptor') # New!
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO users (first_name, last_name, email, password, address, face_descriptor) VALUES (?, ?, ?, ?, ?, ?)",
                  (first_name, last_name, email, password, address, face_descriptor))
        conn.commit()
        user_id = c.lastrowid
...
def api_face_login():
    data = request.json
    login_descriptor_str = data.get('face_descriptor')
    
    if not login_descriptor_str:
        return jsonify({"success": False, "message": "No biometric ID provided"}), 400
    
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT * FROM users WHERE face_descriptor = ?", (login_descriptor_str,))
    ...
    return jsonify({"success": False, "message": "Device Biometrics not recognized"}), 401
...
if __name__ == '__main__':
    print(f"[app] Serving static files from: {FRONTEND_DIR}")
    print(f"[app] Database: {DB_FILE}")
